...
> **Data note:** Built entirely on publicly available HxGN Databridge Pro documentation. No proprietary or confidential company data is stored, indexed, or exposed.
...
- A fully automated 8-stage pipeline that accepts a natural language question and returns a cited, grounded answer
...
- Zero-dependency architecture: all components operate on free tiers (Dify Cloud, Qdrant Cloud free, Groq free, Jina free) with no vendor lock-in — every layer is independently substitutable
...
| LLM — Synthesis | Groq + Qwen3-32B | qwen3-32b, temp=0.1, /no\_think | Grounded answer generation with citations |
| Embeddings | Jina Embeddings v3 | jina-embeddings-v3, 1024-dim, cosine | Dense semantic vector generation |
| Reranker | Jina Reranker v2 | jina-reranker-v2-base-multilingual, top\_n=5 | Cross-encoder relevance rescoring |
| Vector Database | Qdrant Cloud | GCP europe-west3, hxgn\_knowledge\_base | Vector storage and retrieval |
...
| Property | Value |
| --- | --- |
| Collection name | hxgn\_knowledge\_base |
| Total points | 751 |
...
| Dataset 1: technical\_docs | 539 points — chunked markdown from HxGN Databridge Pro documentation |
| Dataset 2: processors\_services | 212 points — NiFi processor and controller service definitions |
| Payload fields (technical\_docs) | text, source, dataset, category, chunk\_index, file\_path |
| Payload fields (processors\_services) | text, source, dataset, category, processor\_name, full\_type, bundle, version, tags, restricted |
...
| Active when | search\_mode == unified | search\_mode == dual | search\_mode == dual |
...
| Input: tech\_results | Tech HTTP node `.body` (string) |
| Input: proc\_results | Proc HTTP node `.body` (string) |
| Output: merged\_results | string — JSON array of combined, sorted point objects |
...
| Input: qdrant\_results | Variable Aggregator `.output` (string) |
| Input: rewritten\_query | Query Rewriter `.text` (string) |
| Output: reranker\_documents | JSON array of passage text strings |
| Output: candidate\_docs | JSON array of full document dicts with metadata |
| Output: query\_for\_reranker | The rewritten query string |
...
| Input: reranker\_response | Jina Reranker `.body` (string) |
| Input: candidate\_docs | Result Mapper `.candidate_docs` (string) |
| Output: context\_passages | Numbered, source-labelled passage block |
| Output: citations\_metadata | JSON array of citation dicts with doc\_id, score, snippet |
...
- **llama-3.1-8b-instant** for Query Rewriting — fast and more than capable for simple query expansion
- **Qwen3-32B** for Answer Synthesis — large, accurate, strong instruction-following needed for grounded citation

This avoids paying the latency cost of a large model where it is not needed.

### 4.8 Zero-Dependency Architecture

The pipeline runs entirely on free-tier services with no vendor lock-in. This was a deliberate design constraint, not just a cost optimisation: every layer (orchestration, embeddings, reranking, vector store, LLM inference) is independently substitutable. Swap Groq for another inference provider, Qdrant for Weaviate, or Jina for OpenAI embeddings — without re-architecting the pipeline. HTTP-first design and stateless request handling are what make this possible.
...
| 4 | `sys.dialogue_turns` variable does not exist in Dify — conversation history injection failed silently | Enable Memory toggle on LLM node with window size 5; Dify handles history injection automatically |
| 5 | RRF fusion scores (hybrid search attempt) in range ~0.01–0.05; result ranking logic assumed 0–1 range — top results incorrectly discarded | Abandoned RRF/hybrid path; switched to dense-only with direct `/points/query`; scores now 0.41–0.52 on relevant queries |
| 6 | Variable Aggregator receiving mismatched types from the two branches (one string, one array) — downstream node type error | Standardised both branches to output string (JSON-serialised); Aggregator output type set to string |
| 7 | Qdrant dual-path search results not merged in score order — lower-quality passages reaching reranker first | Added Merge Results code node to combine and sort by cosine score descending before passing to aggregator |
| 8 | Intent Router reading rewritten query instead of raw query — conceptual questions with processor synonyms routing to dual path incorrectly | Changed Intent Router input from Query Rewriter output to `sys.query` (original user input) |
| 9 | Qwen3-32B `<think>` blocks appearing in final answer output — verbose reasoning polluting user-facing response | Added `/no_think` directive at top of system prompt |
| 10 | Jina Embeddings response `data[0]["embedding"]` occasionally wrapped in extra list layer on certain Dify versions — `json.dumps` of nested list passed to Qdrant | Added explicit flattening check in Extract Vector: `embedding = embedding[0] if isinstance(embedding[0], list) else embedding` |
| 11 | Result Mapper receiving unified path Qdrant response wrapped in `{result: {points: []}}` but treating it as bare list — all results silently dropped | Added isinstance branch in Result Mapper: bare list (dual path) vs wrapped dict (unified path) |
| 12 | Empty `text` field in some NiFi processor payload entries — empty strings reaching reranker and degrading results | Added `if not text: continue` guard in Result Mapper before appending to documents list |
...
The DSL export (`dify-workflow/talk-to-db-pro.yml`) in this repository has all API keys removed and is safe to share publicly.
...
| Citation faithfulness | Source filenames in passage headers; LLM instructed to use filenames not doc\_ids |
...
- Designed and implemented a multi-stage RAG pipeline from scratch on a low-code platform
...
- Integrated four external APIs (Groq, Jina Embeddings, Jina Reranker, Qdrant Cloud) via HTTP with full control over request/response handling
...
- Managed authentication and configuration across all HTTP nodes using environment variables — no hardcoded credentials
...
| Property | Value |
| --- | --- |
| Cluster URL | `https://YOUR_CLUSTER_ID.europe-west3-0.gcp.cloud.qdrant.io:6333` |
